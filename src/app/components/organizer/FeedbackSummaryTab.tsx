...
import type { OrgEvent, FeedbackTheme, FeedbackOpenEndedResult } from './data';
...
function ThemeBadge({ theme }: { theme: FeedbackTheme }) {
  ...
  const s = colors[theme.sentiment];
  ...
  if (!eventId || !event) {
    return (
      <InfoPanel
        ...
        message="Feedback summaries are shown per event. Open a specific event to view the standardized feedback results."
      />
    );
  }
  ...
  if (!isCompletedEvent(event)) {
    return (
      <InfoPanel
        ...
        message="The standardized feedback form is automatically opened after the event has ended. Organizers do not upload or create a separate feedback form."
      />
    );
  }
  ...
  if (!summary) {
    return (
      <InfoPanel
        ...
        message="The event has ended, but no feedback answers have been submitted yet. Once participants answer the standardized form, the rating summary, common themes, and open-ended answers will appear here for both the organizer and CMO."
      />
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1 max-w-2xl" style={{ color: C.muted }}>
            Standardized participant feedback results for this event. This summary is visible to both organizers and CMO.
          </p>
        </div>
      ...
      <div className="rounded-2xl border p-4" style={{ borderColor: C.teal + '40', backgroundColor: C.teal + '08' }}>
        ...
        <p className="text-xs mt-1" style={{ color: C.sub }}>
          The same rating-scale and open-ended questions are used for all events, so CMO and organizers can compare results consistently.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: C.muted }}>Organizers and CMO can view all submitted text answers per open-ended question.</p>
        </div>
